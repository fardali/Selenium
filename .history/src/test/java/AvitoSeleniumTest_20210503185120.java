...
public class AvitoSeleniumTest {
    ...
    public void testAll(){
        ...
        //Matcher emailMatcher = Pattern.compile("\\w{4}\\d{2}@mail.com").matcher(email);
        ...
        //---------------------------------------contact us test---------------------------------------------------------------
        ContactTest contactUs = new ContactTest(this.driver);
        ContactConfirmationPage ccp = contactUs.contact("fardali youssef",email,"this is an automated input sent by selinium test with Java, please ignore this message !");
        String message = ccp.getContactConfirmation();
        Assert.assertEquals("Merci pour votre message !",message);
        //--------------------------------- registration test -------------------------------------------------------------------

        RegisterTest formSend = new RegisterTest(this.driver);
        SendFormResultPage sfrp = formSend.fillFormAndSend("fardali youssef","0600000000","putnythinrandom@gmail.com"); //after running tests twice change the phone number to a random phone number starting with 06XXXXXXXX and a random email
